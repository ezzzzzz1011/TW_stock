...
st.markdown("### 📖 程式內容說明")
st.markdown("""
1. **即時數據抓取**：程式透過 `yfinance` 函式庫，自動在後台連線至 Yahoo Finance 獲取台股最新成交價。
2. **自動代碼補完**：使用者僅需輸入數字代號，系統會自動轉換為 `.TW` 格式進行查詢。
3. **估價邏輯**：採用「本益比估價法」，計算公式為 `EPS × 自訂目標本益比 = 合理參考價`。
4. **狀態判斷**：系統會即時比對「當前股價」與「合理參考價」，並透過顏色標籤提示目前價格是否處於低估區間。
""")
