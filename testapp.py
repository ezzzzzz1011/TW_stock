...
# --- 1. 網頁全域設定 (這必須是程式中第一個執行的 Streamlit 指令) ---
st.set_page_config(
    page_title="台股個股/ETF查詢 Ez開發", page_icon="🔍", layout="wide"
)

# 設定台灣時區
tw_tz = pytz.timezone("Asia/Taipei")

# 建立備用變數，防範原代碼中未定義 all_portfolios 導致 crash
all_portfolios = {}
...
try:
    ...
    try:
        ...
        portfolio_sheet = sh.add_worksheet(
            title="portfolios", rows="1000", cols="2"
        )
    ...
    st.info(
        "💡 提示：請檢查 Google Sheet 是否已分享編輯權限給服務帳號 Email。"
    )
    st.stop()
...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        u_pw = st.text_input(
            "存取密碼", type="password", key="l_pw", placeholder="請輸入密碼"
        )
        ...
        try:
            ...
            if not hist.empty:
                ...
                try:
                    ...
                    soup = BeautifulSoup(
                        requests.get(name_url, headers=headers, timeout=5).text,
                        "html.parser",
                    )
...
if st.session_state.page == "home":
    ...
    with col_a:
        st.subheader("📈 個股分析")
        if st.button(
            "個股查詢與估價", use_container_width=True, type="primary"
        ):
            go_to("stock_query")
    with col_b:
        st.subheader("📊 ETF 分析")
        if st.button(
            "ETF 試算與規劃", use_container_width=True, type="primary"
        ):
            go_to("etf_query")
    with col_c:
    ...
    with col_d:
        st.subheader("💼 我的資產")
        if st.button(
            "個人投資組合", use_container_width=True, type="primary"
        ):
            go_to("portfolio")
...
# ==========================================
    ...
    with main_col:
        ...
        if stock_code:
            ...
            if info:
                ...
                with cp1:
                    ...
                    st.markdown(
                        f"<span style='color:{color}; font-weight:bold; font-size:1.5rem;'>{info['change']:+.2f} ({info['pct']:+.2f}%)</span>",
                        unsafe_allow_html=True,
                    )
                with cp2:
                    st.caption("今日行情細節")
                    ...
                    st.write(
                        f"開盤: {info['open']:.2f} / 總量: {int(info['vol']/1000):,} 張"
                    )
                st.divider()
        ...
        with col_eps:
            eps = st.number_input(
                "輸入該股 EPS (4季累積)", min_value=0.01, step=0.1, value=10.0
            )
        with col_pe:
            pe_target = st.number_input(
                "自訂參考本益比 (PE)", value=15.0, step=0.1
            )
        ...
        if current_price > 0:
            ...
            st.subheader("📊 換算結果")
            st.markdown(
                f"<div class='calc-box'>合理價參考：<span class='highlight-val'>{fair_price:.2f}</span></div>",
                unsafe_allow_html=True,
            )
...
# ==========================================
    ...
    with main_col:
        ...
        with input_c1:
            symbol_input = (
                st.text_input("ETF 代號", placeholder="例如: 00919")
                .strip()
                .upper()
            )
        with input_c2:
        ...
        if st.session_state.data and st.session_state.data.get("success"):
            ...
            st.markdown(
                f"## {d['name']} <small style='font-size:1rem; color:#aaa;'>(偵測為{d['freq_label']}配息)</small>",
                unsafe_allow_html=True,
            )
            st.markdown(
            ...
            with info_c1:
                ...
                st.markdown(
                    f"<span style='color:{m_color}; font-weight:bold; font-size:1.5rem;'>{d['change']:+.2f} ({d['pct']:+.2f}%)</span>",
                    unsafe_allow_html=True,
                )
            with info_c2:
                st.caption("今日行情細節")
                ...
                st.write(
                    f"開盤: {d['open']:.2f} / 總量: {d['vol']/1000:,.0f} 張"
                )

            st.divider()
            st.subheader("📑 歷史配息參考")
            e_cols = st.columns(4)
            d1 = e_cols[0].number_input(
                "最新", value=float(d["raw_divs"][0]), format="%.3f"
            )
            d2 = e_cols[1].number_input(
                "前一", value=float(d["raw_divs"][1]), format="%.3f"
            )
            d3 = e_cols[2].number_input(
                "前二", value=float(d["raw_divs"][2]), format="%.3f"
            )
            d4 = e_cols[3].number_input(
                "前三", value=float(d["raw_divs"][3]), format="%.3f"
            )
            ...
            with stat_c1:
                ...
                st.markdown(
                    f"<div class='highlight-val'>{avg_annual:.2f}</div>",
                    unsafe_allow_html=True,
                )
            with stat_c2:
                st.caption("實質殖利率")
                st.markdown(
                    f"<div class='highlight-val'>{real_yield:.2f}%</div>",
                    unsafe_allow_html=True,
                )
            ...
            st.markdown(
                f"<div class='calc-box'>系統建議：<b>{rec}</b></div>",
                unsafe_allow_html=True,
            )
            ...
            with calc_c1:
                hold_lots = st.number_input(
                    "持有張數", min_value=0, value=10, step=1
                )
            with calc_c2:
                ...
                nhi_amt = (
                    div_54c_part * 0.0211 if div_54c_part >= 20000 else 0
                )
            ...
            with st.container():
                ...
                with f_col0:
                    custom_initial = st.number_input(
                        "初始投入總金額 (元)",
                        min_value=0,
                        value=100000,
                        step=10000,
                ...
                with f_col2:
                    custom_yield = st.number_input(
                        "自訂年化殖利率 (%)",
                        value=float(f"{real_yield:.2f}"),
                        step=0.1,
                    )
                with f_col3:
...
# ==========================================
    ...
    with col_in1:
        code1 = st.text_input("輸入代碼 A", value="00919").strip().upper()
    with col_in2:
        code2 = st.text_input("輸入代碼 B", value="00878").strip().upper()
        ...
        with st.spinner("抓取對比數據中..."):
            ...
            if r1["success"] and r2["success"]:
                ...
                for r in [r1, r2]:
                    ...
                    real_yield = (
                        (avg_annual / r["price"]) * 100 if r["price"] > 0 else 0
                    )
                    analysis.append(
                        {"annual_div": avg_annual, "yield": real_yield}
                    )
...
# ==========================================
    ...
    with st.expander("📥 匯入投資清單 (CSV)"):
        ...
        if uploaded_file is not None:
            try:
                ...
                if (
                    "代碼" in import_df.columns
                    and "張數" in import_df.columns
                ):
                    # 抓取檔案中的資料
                    ...
                    if len(new_data) < 20:
                        padding = pd.DataFrame(
                            [
                                {"代碼": "", "張數": None}
                                for _ in range(20 - len(new_data))
                            ]
                        )
                        new_data = pd.concat(
                            [new_data, padding], ignore_index=True
                        )

                    st.session_state.portfolio = new_data
                    st.success(
                        "CSV 資料已載入編輯器，請檢查後點擊下方儲存按鈕。"
                    )
                else:
                    st.error(
                        "CSV 格式錯誤！必須包含『代碼』與『張數』兩個欄位。"
                    )
            except Exception as e:
                st.error(f"讀取失敗：{e}")
    ...
    if len(st.session_state.portfolio) < 20:
        padding_df = pd.DataFrame(
            [
                {"代碼": "", "張數": None}
                for _ in range(20 - len(st.session_state.portfolio))
            ]
        )
        st.session_state.portfolio = pd.concat(
            [st.session_state.portfolio, padding_df], ignore_index=True
        )

    # 顯示編輯器 (已修正縮排，確保只出現在此頁)
    ...
    with col1:
        if st.button(
            "💾 儲存至雲端", type="primary", use_container_width=True
        ):
            # 更新 session_state
            st.session_state.portfolio = edited_df
            # 呼叫我們先前定義好的存檔函數
            try:
                save_portfolio_to_cloud(
                    st.session_state.current_user, edited_df
                )
    ...
    with col2:
        st.info(
            "💡 編輯完代碼或張數後，請務必點擊左側儲存按鈕，帳號重啟後資料才不會消失。"
        )
    ...
    valid_df["代碼"] = valid_df["代碼"].astype(str).str.strip()
    valid_df = valid_df[valid_df["代碼"] != ""]
    ...
    if not valid_df.empty:
        ...
        if results:
            ...
            avg_yield = (
                (total_annual_div / total_market_val * 100)
