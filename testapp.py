...
# ==========================================
# 🌟 全新精準配息爬蟲 (Yahoo 台灣 JSON 官方來源)
# ==========================================
def get_dividend_info(symbol):
    """
    抓取官方配息數據，回傳完整的 DataFrame 以供精準判斷配息頻率與金額。
    """
    clean_symbol = str(symbol).strip().upper().replace('.TW', '').replace('.TWO', '')
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
    div_history = []
    
    # 策略 1: 使用最精準的 Yahoo Taiwan API (不受版面結構影響)
    for suffix in ['.TW', '.TWO', '']:
        try:
            ...
            if res.status_code == 200:
                ...
                if data:
                    for item in data:
                        dt = item.get('exDividendAppointedDay', '')[:10]
                        amt = item.get('cashDividend')
                        if dt and amt is not None:
                            div_history.append({'date': pd.to_datetime(dt), 'amount': float(amt)})
                    if div_history:
                        break
        except:
            continue
            
    # 策略 2: 如果 Yahoo API 沒抓到，嘗試用原本的 yfinance 備援
    if not div_history:
        try:
            for suffix in ['.TW', '.TWO']:
    ...
    if div_history:
        df = pd.DataFrame(div_history).sort_values('date', ascending=False)
        return df
    return pd.DataFrame()
...
def get_safe_data_etf(symbol):
    ...
    try:
        # 使用精準的配息獲取函數
        df_divs = get_dividend_info(symbol)
        
        if not df_divs.empty:
            # 取得最近 4 次配息
            d_list = df_divs['amount'].head(4).tolist()
            while len(d_list) < 4: 
                d_list.append(0.0)
            raw_divs = d_list
            
            # 判斷配息頻率 (月/季/半年/年) - 準確計算過去 365 天內的發放次數
            last_year_date = pd.to_datetime(datetime.now()) - pd.DateOffset(years=1)
            count_in_year = len(df_divs[df_divs['date'] > last_year_date])
...
def get_dividend_calendar(symbol):
    """抓取單一股票的除息日與發放日預估 (串接精準配息爬蟲)"""
    clean_symbol = str(symbol).strip().upper().replace('.TW', '').replace('.TWO', '')
    
    df_divs = get_dividend_info(symbol)
    
    if not df_divs.empty:
        latest = df_divs.iloc[0]
        amount = latest['amount']
        ex_date_str = latest['date'].strftime('%Y-%m-%d')
        pay_date = (latest['date'] + pd.DateOffset(days=30)).strftime('%Y-%m-%d')
