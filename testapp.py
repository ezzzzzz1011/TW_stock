...
def get_etf_holdings(symbol):
    try:
        ...
        for item in items:
            ...
            # 偵測包含百分比的行
            text_content = [d.get_text(strip=True) for d in divs if d.get_text(strip=True)]
            if len(text_content) >= 3 and "%" in text_content[2]:
                name = text_content[0]
                ratio = text_content[2]
                if name != "成分股名稱":
                    # 清洗名稱，去掉代號
...
if st.session_state.page == "main":
    ...
    if st.button("⬅️ 返回計算機"):
        ...
        st.rerun()
    st.divider()
    with st.spinner('正在獲取最新成分股資料...'):
        holdings_df = get_etf_holdings(data['symbol'])
        if holdings_df is not None:
            st.markdown("### 🏆 前十大權重持股")
            st.table(holdings_df)
            st.info("註：資料抓取自 Yahoo 財經，權重比例僅供參考。")
        else:
            st.warning("暫時無法取得成分股資料。請確認代號是否為 ETF。")
